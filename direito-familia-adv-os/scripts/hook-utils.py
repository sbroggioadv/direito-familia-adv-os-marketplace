...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando direito-familia/ acima
- hook_state_load/save    → gerencia direito-familia/.hook-state.json (debounce anti-flap)
...
STATE_DIR = "direito-familia"
HOOK_STATE_FILENAME = ".hook-state.json"


def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/direito-familia/cowork-state.json."""
    start = start.resolve()
